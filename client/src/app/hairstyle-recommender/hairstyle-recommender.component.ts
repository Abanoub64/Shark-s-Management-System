...
@Component({
  ...
  styleUrl: './hairstyle-recommender.component.css'
})
export class HairstyleRecommenderComponent {
  ...
  // ... (File handling methods remain the same as they are standard logic) ...
  ...
  private processFile(file: File) {
    if (!file.type.startsWith('image/')) {
      this.error.set('Please upload a valid image file.');
      return;
    }
  ...
  generateHairstyle() {
    ...
    this.lightXService.generateHairstyle(this.selectedFile, selectedType.prompt)
      .subscribe({
        ...
        error: (err) => {
          console.error(err);
          this.error.set(err.message || 'Failed to generate hairstyle');
          this.isLoading.set(false);
        }
      });
  }
