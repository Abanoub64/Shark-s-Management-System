...
@Component({
  ...
  template: `
    ...
    <section class="relative min-h-[600px] flex items-center bg-black overflow-hidden">
      ...
      <div class="container mx-auto px-4 relative z-10 pt-20">
        <div class="max-w-3xl">
          ...
          <h1 class="text-5xl md:text-7xl lg:text-8xl font-bold text-white mb-8 leading-tight font-serif tracking-tight">
            {{ t().experienceArt }} <br />
            <span
          ...
          <div class="flex flex-col sm:flex-row gap-5">
            <a
              routerLink="/branches"
              class="inline-flex items-center justify-center rounded-md font-medium transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-secondary w-full sm:w-auto bg-gradient-to-r from-secondary to-yellow-400 hover:from-yellow-400 hover:to-secondary text-black font-bold border-none shadow-[0_0_20px_rgba(212,175,55,0.4)] transform hover:scale-105 text-lg py-4 px-8"
            >
              {{ t().bookAppointment }}
            </a>
          </div>
          ...
          <div class="mt-16 flex gap-12 border-t border-white/10 pt-10">
            <div>
              ...
              <p class="text-sm text-gray-400 uppercase tracking-widest mt-1">{{ t().averageRating }}</p>
            </div>
            <div>
              <p class="text-4xl font-bold text-secondary font-serif">15k+</p>
              <p class="text-sm text-gray-400 uppercase tracking-widest mt-1">{{ t().happyClients }}</p>
            </div>
            <div>
              <p class="text-4xl font-bold text-secondary font-serif">10+</p>
              <p class="text-sm text-gray-400 uppercase tracking-widest mt-1">{{ t().yearsExperience }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="py-24 bg-black text-white relative">
      <!-- Decorative Elements -->
      <div class="absolute top-0 right-0 w-1/3 h-full bg-gradient-to-l from-zinc-900 to-transparent opacity-50 pointer-events-none"></div>
      ...
      <div class="container mx-auto px-4 relative z-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-10">
          <div class="text-center p-8 rounded-3xl bg-zinc-900/50 border border-white/5 hover:border-secondary/30 transition-all duration-300 hover:transform hover:-translate-y-2 group">
            <div
    ...
    <section class="py-24 bg-zinc-950 relative border-t border-white/5">
      <div class="container mx-auto px-4">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
          ...
          <a routerLink="/branches" class="text-secondary font-medium hover:text-white transition-colors flex items-center gap-2 group"
            >{{ t().viewAll }} <span class="group-hover:translate-x-1 transition-transform">-></span></a
          >
        </div>
          ...
          >
            ...
            <div class="absolute top-6 left-6 right-6 flex justify-between items-start z-10">
              ...
              <span
                ...
                [class.bg-green-500/90]="branch.isOpen !== false"
                [class.text-white]="branch.isOpen !== false"
                [class.bg-red-500/90]="branch.isOpen === false"
                [class.text-white]="branch.isOpen === false"
              >
            ...
            <div class="absolute bottom-0 left-0 right-0 p-8 z-10">
              <h3 class="text-3xl font-bold text-white mb-3 leading-tight font-serif group-hover:text-secondary transition-colors">
                {{ branch.name }}
              </h3>

              <div class="flex items-center gap-2 text-gray-300 mb-6 text-sm border-b border-white/10 pb-4">
              ...
              <div class="flex items-center justify-between">
                 <div class="flex flex-col">
                   ...
                   <span class="text-sm text-white font-medium">{{ branch.hours || '10:00 AM - 10:00 PM' }}</span>
                 </div>

                 <span class="w-10 h-10 rounded-full bg-secondary flex items-center justify-center text-black transform group-hover:rotate-45 transition-transform duration-300">
    ...
    <button
      ...
      [class.translate-y-10]="!showScrollTop"
    >
